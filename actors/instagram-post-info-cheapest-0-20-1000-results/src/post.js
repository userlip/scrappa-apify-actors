...
export async function fetchPost(get, params, apiKey, onFallback = () => {}) {
    ...
    try {
        ...
        const loginRequired = error.response?.status === 403
            && error.response?.data?.error_code === 'instagram_login_required';
        if (!identity || options.signal.aborted || (!loginRequired && !isTransientScrappaError(error))) throw error;
