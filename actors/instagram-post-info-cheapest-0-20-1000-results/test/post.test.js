...
test('does not mask authentication errors or use feed fallback for bare shortcodes', async () => {
    for (const [params, error] of [
        ...
        [{ url }, { response: { status: 403, data: { error_code: 'invalid_api_key' } } }],
...
test('uses exact feed match for Instagram login-required responses without retrying login', async () => {
    let calls = 0;
    const post = { shortcode: 'Dc30nJeRKKz' };
    const response = await fetchPost(async () => {
        if (++calls === 1) throw { response: { status: 403, data: {
            success: false, error_code: 'instagram_login_required', retryable: false,
        } } };
        return { data: { success: true, posts: [post] } };
    }, { url }, 'test');
    assert.deepEqual(response.data, { success: true, found: true, data: post });
