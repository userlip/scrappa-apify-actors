...
import { buildPlaylistDetailsUrl } from './playlist-url.js';
...
Actor.main(async () => {
    try {
        ...
        const apiUrl = buildPlaylistDetailsUrl(input);
        ...
        console.log(`Successfully fetched ${Array.isArray(data) ? data.length : 1} playlist detail result(s) for id: ${input.id}`);
        ...
        console.error(`Failed to fetch YouTube playlist details: ${message}`);
