...
fn autocomplete_url_keeps_the_query_and_encodes_it_for_scrappa() {
    ...
    let (query, url) = autocomplete_url(&json!({"query": "time sq, New York"}), &base_url).unwrap();
...
async fn scrappa_request_deadline_stops_a_slow_request_without_retry() {
    ...
    let error =
        fetch_suggestions_with_timeout(&Client::new(), &config, &url, Duration::from_millis(20))
            .await
            .unwrap_err()
            .to_string();
...
async fn actor_preserves_auth_dataset_rows_output_and_pay_per_event_budget() {
    ...
    let request_url = Url::parse(&format!("http://mock{}", scrappa_requests[0].target)).unwrap();
    assert_eq!(
