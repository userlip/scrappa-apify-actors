...
const chargingModule = process.env.TEST_SOURCE === 'src'
    ...
    : '../dist/charging.js';
const {
    COMPANY_DETAIL_RESULT_CHARGE_EVENT,
    pushChargedItems,
...
test('pushes paid-mode items with company detail result charge event', async () => {
    ...
    assert.deepEqual(result, { savedCount: 2, statusMessage: null });
    assert.equal(dataset.calls.length, 1);
    assert.equal(dataset.calls[0].eventName, COMPANY_DETAIL_RESULT_CHARGE_EVENT);
});
