...
pub fn build_request_plan(input: &Value) -> Result<RequestPlan, String> {
    ...
    if targets.is_empty() {
        return Err(
            "Provide at least one Trusted Shops TSID or profile URL in tsids or urls".into(),
        );
    }
    if targets.len() > MAX_TARGETS_PER_RUN {
        return Err(format!(
            "Provide {MAX_TARGETS_PER_RUN} or fewer Trusted Shops targets per run"
        ));
    }

    let start_page = clean_integer(input.get("page"), "page", 1, MAX_PAGE)?.unwrap_or(1);
    let max_pages =
        clean_integer(input.get("max_pages"), "max_pages", 1, MAX_PAGES_PER_TARGET)?.unwrap_or(1);
    ...
    let include_raw_responses =
        clean_boolean(input.get("include_raw_responses"), "include_raw_responses")?
            .unwrap_or(false);
...
fn clean_string_array(value: Option<&Value>, field: &str) -> Result<Vec<String>, String> {
    ...
    Ok(clean_optional_string(Some(value), field, 500)?
        .into_iter()
        .collect())
}
...
mod tests {
    ...
    fn builds_batch_targets_and_page_parameters() {
        ...
        assert_eq!(
            ...
            "1 Trusted Shops target(s) (pages 2-4)"
        );
    }
    ...
    fn extracts_tsids_from_direct_values_and_urls() {
        assert_eq!(
            extract_tsid(&TSID.to_ascii_lowercase()).as_deref(),
            Some(TSID)
        );
        assert_eq!(
            extract_tsid(&format!("www.trustedshops.de/bewertung/info_{TSID}.html")).as_deref(),
            Some(TSID)
        );
        assert_eq!(
            extract_tsid(&format!(
                "https://example.com/api/trustedshops/reviews/{TSID}"
            ))
            .as_deref(),
            Some(TSID)
        );
        assert_eq!(
            extract_tsid(&format!("https://example.com/%E0%A4%A/info_{TSID}.html")).as_deref(),
            Some(TSID)
        );
    }
    ...
    fn supports_single_input_compatibility_and_defaults() {
        let plan = build_request_plan(
            &json!({ "url": format!("https://www.trustedshops.eu/buyerrating/info_{TSID}.html") }),
        )
        .unwrap();
        assert_eq!(plan.targets[0].tsid, TSID);
        assert_eq!(
            plan.targets[0].source_url,
            format!("https://www.trustedshops.eu/buyerrating/info_{TSID}.html")
        );
    ...
    fn reports_the_same_validation_errors() {
        assert_eq!(
            build_request_plan(&json!({})).unwrap_err(),
            "Provide at least one Trusted Shops TSID or profile URL in tsids or urls"
        );
        assert!(
            build_request_plan(&json!({ "tsids": ["not-a-tsid"] }))
                .unwrap_err()
                .contains("must contain a 33-character Trusted Shops TSID")
        );
        assert_eq!(
            build_request_plan(&json!({ "tsids": [TSID], "max_pages": 26 })).unwrap_err(),
            "max_pages must be between 1 and 25"
        );
        assert_eq!(
            build_request_plan(&json!({ "tsids": [TSID], "page": 99, "max_pages": 3 }))
                .unwrap_err(),
            "page plus max_pages cannot exceed page 100"
        );
        assert!(
            build_request_plan(&json!({ "tsids": [TSID], "market": "USA" }))
                .unwrap_err()
                .starts_with("market must be one of:")
        );
        assert_eq!(
            build_request_plan(&json!({ "tsids": [TSID], "include_raw_responses": "true" }))
                .unwrap_err(),
    ...
    fn validates_batch_limit_and_string_inputs() {
        let tsids = (0..51)
            .map(|index| json!(format!("{index:033X}")))
            .collect::<Vec<_>>();
        assert_eq!(
            build_request_plan(&json!({ "tsids": tsids })).unwrap_err(),
            "Provide 50 or fewer Trusted Shops targets per run"
        );
        assert_eq!(
            build_request_plan(&json!({ "tsids": 42 })).unwrap_err(),
            "tsids must be a string"
        );
    }
}
