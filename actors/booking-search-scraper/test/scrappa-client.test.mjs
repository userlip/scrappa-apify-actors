...
test('builds GET requests with filtered params and actor user agent', async () => {
    ...
    try {
        ...
        const result = await client.get('/booking/search', {
            ss: 'Paris',
            use_cache: true,
            debug: false,
            empty: '',
        ...
        assert.equal(url.pathname, '/api/booking/search');
        assert.equal(url.searchParams.get('ss'), 'Paris');
        ...
        assert.equal(capturedOptions.headers['User-Agent'], 'thescrappa-booking-search-scraper/1.0');
    } finally {
...
test('parses Scrappa JSON error messages and validation details', async () => {
    ...
    globalThis.fetch = async () => new Response(
        JSON.stringify({
            ...
            errors: {
                ss: ['The destination is required.'],
            },
        }),
...
test('retries transient fetch network failures', async () => {
    ...
    try {
        ...
        const result = await client.get('/booking/search', {}, { attempts: 2 });
