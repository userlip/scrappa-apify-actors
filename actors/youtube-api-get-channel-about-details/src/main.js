...
async function getChannelAboutDetails(id, apiKey) {
    const { apiUrl, requestOptions } = buildScrappaRequest(buildChannelAboutDetailsUrl(id), apiKey);
    try {
...
Actor.main(async () => {
    ...
    for (const id of ids) {
        try {
            const data = await getChannelAboutDetails(id, apiKey);
