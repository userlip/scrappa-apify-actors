...
export function isRetryableScrappaError(error: unknown): boolean {
    ...
    if (error instanceof TypeError && /fetch failed|failed to fetch|network|terminated|reset|econnrefused|econnreset|socket hang up|chunk/i.test(error.message)) {
        return true;
    }
...
export class ScrappaClient {
    ...
    private async send<T>(
        ...
        const headers: Record<string, string> = {
            ...
            'User-Agent': 'thescrappa-google-trends-autocomplete-scraper/1.0',
        };
