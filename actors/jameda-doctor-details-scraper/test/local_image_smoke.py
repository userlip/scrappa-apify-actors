    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            if self.path == "/api/v2/actor-runs/smoke-run/charge":
                if not state.dataset_items:
                    return json_response(self, 409, {"message": "dataset row must be persisted before its custom charge"})
...
def main():
    ...
    try:
        ...
        print("Local image smoke passed: authenticated request, dataset-first PPE charge, and OUTPUT record verified.")
    finally:
