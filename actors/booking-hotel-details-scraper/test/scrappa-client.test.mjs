...
test('builds GET requests with filtered params and actor user agent', async () => {
    ...
    try {
        ...
        const result = await client.get('/booking/hotel', {
            country: 'fr',
            slug: 'ritz-paris',
            use_cache: true,
            debug: false,
        ...
        assert.equal(url.pathname, '/api/booking/hotel');
        assert.equal(url.searchParams.get('country'), 'fr');
        assert.equal(url.searchParams.get('slug'), 'ritz-paris');
        ...
        assert.equal(capturedOptions.headers['User-Agent'], 'thescrappa-booking-hotel-details-scraper/1.0');
    } finally {
...
test('parses Scrappa JSON error messages and validation details', async () => {
    ...
    try {
        ...
        await assert.rejects(
            () => client.get('/booking/search'),
            /Scrappa API error \(422\): Invalid request - slug: The slug is required\./,
        );
    } finally {
        globalThis.fetch = originalFetch;
    }
...
test('retries transient fetch network failures', async () => {
    ...
    try {
        ...
        const result = await client.get('/booking/hotel', {}, { attempts: 2 });
