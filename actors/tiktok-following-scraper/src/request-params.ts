...
export function formatTikTokFollowingLookupForLog(input: TikTokFollowingInput): string {
    return resolveTikTokFollowingLookup(input)?.logValue ?? 'unknown TikTok profile';
}
...
function resolveTikTokFollowingLookup(
...
export function buildTikTokFollowingParams(
    input: TikTokFollowingInput,
    warn: (message: string) => void = console.warn,
): Record<string, unknown> {
    const params: Record<string, unknown> = {};

    const lookup = resolveTikTokFollowingLookup(input, warn);
    if (lookup) {
