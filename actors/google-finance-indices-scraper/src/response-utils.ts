export interface GoogleFinanceIndexRow {
    ...
    price_movement_direction?: unknown;
    [key: string]: unknown;
}

export interface GoogleFinanceIndicesResponse {
    data?: unknown;
    indices?: unknown;
    results?: unknown;
    [key: string]: unknown;
...
export function canonicalSymbol(value: unknown): string | null {
...
export function mapIndexRow(
    ...
    const id = `${exchange?.toUpperCase() ?? 'UNKNOWN'}:${symbol}`;
    ...
    return {
        ...
        exchange,
        current_price: number(row.current_price ?? row.price),
        price_change: number(row.price_change ?? row.change),
        percent_change: number(row.percent_change ?? row.price_change_percent),
        previous_close: number(row.previous_close),
        movement_direction: string(row.movement_direction ?? row.price_movement_direction),
