...
export interface BatchDependencies { getCapacity(): number; fetch(): Promise<GoogleFinanceIndicesResponse>; save(item: IndexItem): Promise<{ savedCount: number; chargeLimitReached: boolean }>; }
export interface BatchSummary { requested: number; attempted: number; saved: number; duplicate: number; failed: number; charged: number; charge_limit_reached: boolean; outcomes: Array<{ symbol: string; status: string; error?: string }>; }
export async function runIndicesBatch(requested: string[], params: { hl: string; gl: string }, dependencies: BatchDependencies): Promise<BatchSummary> {
    const outcomes: BatchSummary['outcomes'] = []; let saved = 0; let duplicate = 0; let failed = 0; let limited = false; const seen = new Set<string>();
    ...
    for (const row of rows) {
        ...
        const result = await dependencies.save(item); if (result.savedCount === 1) { seen.add(item.id); saved += 1; outcomes.push({ symbol: sourceSymbol, status: 'saved' }); } else { failed += 1; outcomes.push({ symbol: sourceSymbol, status: 'failed', error: 'Apify did not save a chargeable index result' }); }
        if (result.chargeLimitReached) { limited = true; break; }
    }
    for (const symbol of requested) if (!outcomes.some((outcome) => outcome.symbol === symbol)) { failed += 1; outcomes.push({ symbol, status: 'failed', error: 'Scrappa returned no matching index result' }); }
    return { requested: requested.length, attempted: 1, saved, duplicate, failed, charged: saved, charge_limit_reached: limited, outcomes };
}
