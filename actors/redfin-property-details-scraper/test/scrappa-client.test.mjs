...
test('builds GET requests with filtered params and actor user agent', async () => {
    ...
    try {
        ...
        const result = await client.get('/redfin/property', {
        ...
        assert.equal(url.pathname, '/api/redfin/property');
        assert.equal(url.searchParams.get('property_id'), '60791456');
        ...
        assert.equal(capturedOptions.headers['User-Agent'], 'thescrappa-redfin-property-details-scraper/1.0');
    } finally {
    ...
    try {
        ...
        await assert.rejects(
            ...
            (error) => {
                ...
                assert.equal(error.message, 'Scrappa API error (422): Invalid request - property_id: The property ID is required.');
                return true;
            },
        );
    } finally {
