...
async function getChannelStatistics(id) {
    ...
    const apiUrl = `https://ytapi.scrappa.co/channels/stats?id=${encodeURIComponent(id)}`;
...
Actor.main(async () => {
    ...
    for (const id of ids) {
        const data = await getChannelStatistics(id);
