...
| `actors/google-trends-autocomplete-scraper` | `google-trends-autocomplete-scraper` | `x4NPccdc4xbJZ2r6c` | Google Trends Autocomplete Scraper | Local source present; live build `VXacHkn44mXNMgp29` / `1.0.3` succeeded; q-alias smoke run `ZLbwe79uti9GUqanB` succeeded with 5 suggestion rows for `tesla` in `US`; default dataset `FjAtuYTzJDibf6GBK`; `OUTPUT.saved_suggestion_count = 5`; `SCRAPPA_API_KEY` is secret on version `1.0` | Public with `PAY_PER_EVENT` pricing scheduled for 2026-06-19T09:30:30.582Z, `suggestion-result` at $0.0002/result; Apify blocked immediate pricing with two-week lead time |
