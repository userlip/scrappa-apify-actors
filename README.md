...
This inventory is aligned to the live `TheScrappa` Apify org as of 2026-05-26T19:25:00.000Z. It lists all 77 live `thescrappa` actors. Actor versions use Apify `SOURCE_FILES`; the live metadata does not indicate a Git-linked Apify source.

Current coverage in this repository:
- 77 live `thescrappa` actors in Apify
- 64 public `thescrappa` actors in Apify according to the pricing and secret audits
- 77 local actor source directories in this repo; all 77 are represented by live Apify actors
...
- Pricing follow-up: the 2026-05-26T19:25:00.000Z pricing audit found 0 public actors missing paid pricing, 0 overdue active-pricing gaps, 52 public actors with active paid evidence, and 12 public actors with future-only paid pricing.
- Secret follow-up: the 2026-05-26T19:19:56.817Z secret audit found all 64 public actors have `SCRAPPA_API_KEY` configured as a secret.
...
| `actors/redfin-property-details-scraper` | `redfin-property-details-scraper` | `9DsqUcpBF5b66AFOY` | Redfin Property Details Scraper | Local source imported from live Apify `SOURCE_FILES` on 2026-05-26; latest run `ZPPyddSY5tXutNft5` succeeded; latest build `1.0.2` present | Private actor; no paid `pricingInfos` reported by Apify API while private |
...
| `actors/redfin-valuation-scraper` | `redfin-valuation-scraper` | `g4let6UdMxr2Io4uc` | Redfin Valuation Scraper | Local source imported from live Apify `SOURCE_FILES` on 2026-05-26; latest run `j6lw0618bXgEGpKgm` succeeded; latest build `1.0.1` present | Private actor with `pricingInfos` active from 2026-05-26T19:20:00.000Z (`PAY_PER_EVENT`); verify before publication |
...
| `actors/website-content-extractor-scraper` | `website-content-extractor-scraper` | `bAUjzVta9rv6bpa10` | Website Content Extractor Scraper | Local source imported from live Apify `SOURCE_FILES` on 2026-05-26; no latest build or run returned by Apify API yet | Private actor; no paid `pricingInfos` reported by Apify API while private |
