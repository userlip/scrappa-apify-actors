...
| `actors/google-trends-autocomplete-scraper` | `google-trends-autocomplete-scraper` | `x4NPccdc4xbJZ2r6c` | Google Trends Autocomplete Scraper | Local source present; live build `qOOP2FPPA0nHE1klU` / `1.0.2` succeeded; smoke run `RINdV5VYPzjfj42Fe` succeeded with 5 suggestion rows for `tesla` in `US`; default dataset `HJVYxriINaR0ReY3o`; `SCRAPPA_API_KEY` is secret on version `1.0` | Public with `PAY_PER_EVENT` pricing scheduled for 2026-06-19T09:30:30.582Z, `suggestion-result` at $0.0002/result; Apify blocked immediate pricing with two-week lead time |
