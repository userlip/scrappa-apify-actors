...
|---|---|---|---|---|---|
| `actors/arbeitsagentur-jobs-scraper` | `arbeitsagentur-jobs-scraper` | `EiUCYz2MjYUuGT6Xu` | Arbeitsagentur Jobs Scraper ($0.30/1k results) | Local source present; live Apify version uses `SOURCE_FILES`; headerless HTTP 503 retry pacing deployed as build `1.0.7`; exact-prefill run `L7ephhqN1gjyt0uca` succeeded with 25 dataset items in 2.476 seconds wall time; maintenance notice cleared 2026-09-05 | Active `PAY_PER_EVENT`, default dataset job result at $0.0003/result |
| `actors/booking-search-scraper` | `booking-search-scraper` | `BehWN3LEvBxhEiJDF` | Booking.com Search Scraper | Local source present; live Apify version uses `SOURCE_FILES`; latest inspected run `fGubXwuYYNmoMyakt` succeeded with 28 dataset items; stale Apify maintenance notice cleared 2026-05-30 | Scheduled 2026-06-03 (`PAY_PER_EVENT`, `booking-result` at $0.0002/result); Apify pricing schedule created 2026-05-20 with two-week lead time |
...
| `actors/google-finance-intraday-scraper` | `google-finance-intraday-scraper` | `IeOWC1bvG8SUebyDa` | Google Finance Intraday Scraper | Local source present; live Apify version uses `SOURCE_FILES`; latest run `cpq3mz8eLiq0e2q1R` succeeded; latest build `XTQDfa0WCGu1xFHnB` succeeded | Public with `PAY_PER_EVENT` pricing scheduled for 2026-06-12T22:30:49.449Z, `intraday-price-point` at $0.0002/result; Apify pricing schedule created 2026-05-29 with two-week lead time |
...
| `actors/google-flights-search-scraper` | `google-flights-search-scraper` | `IIPXRhbeyXH7ssOK6` | Google Flights Search Scraper | Local source present; live Apify version uses `SOURCE_FILES`; QA retry/date hardening deployed as build `1.0.5`; exact-prefill run `g45XRdNGeJYZool37` succeeded with 32 results in 2.655 seconds | `pricingInfos` start 2026-05-11 (`PAY_PER_EVENT`, `flight-result` at $0.0002/result) |
| `actors/google-hotels-search-scraper` | `google-hotels-search-scraper` | `Kc3rfsV2Hif23mctw` | Google Hotels Search Scraper | Local source present; live Apify version uses `SOURCE_FILES`; smoke run `0kc6kUen9F8Z53MAK` succeeded with 20 dataset items; stale Apify maintenance notice cleared 2026-05-30; local source now writes default dataset items without the obsolete explicit `hotel-result` event | Scheduled 2026-05-26 (`PAY_PER_EVENT`, default dataset hotel result at $0.0002/result); verify deployed source no longer logs unknown charge event |
...
| `actors/google-videos-scraper` | `google-videos-scraper` | `kAdTwn5fkBCGKOQUq` | Google Videos Scraper | Local source present; live Apify version uses `SOURCE_FILES`; smoke run `oGeUylDZScdeKwCF1` succeeded with 10 dataset items; stale Apify maintenance notice cleared 2026-05-30 | Scheduled 2026-05-31 (`PAY_PER_EVENT`, default dataset video result at $0.0002-$0.0003/result by tier); Apify blocked immediate pricing with two-week lead time |
...
| `actors/google-maps-directions-scraper` | `google-maps-directions-scraper` | `ZF8jFdzF15k49AZQh` | Google Maps Directions Scraper | Local source present; QA recovery build `V8RvkolpoBumvavCu` / `1.0.8` succeeded; QA-style default run `hDKkj4p4DCHcrjaSi` produced 3 route rows; explicit two-route run `ptWnOlygGojAGBWQ9` produced exactly 6 rows; Actor notice verified as `NONE` | Active `PAY_PER_EVENT` pricing at `$0.0005` per `route-result`; dataset rows and charges matched in both recovery runs |
...
| `actors/google-patents-details-scraper` | `google-patents-details-scraper` | `O7u78wndFctfx6KvD` | Google Patents Details Scraper | Local source present; live Apify version uses `SOURCE_FILES`; latest run `oYfsDpTjav2cRa81X` succeeded; latest build `0cTdsWzOYKJOnL07O` succeeded | Public with `PAY_PER_EVENT` pricing scheduled for 2026-06-09T20:28:35.704Z, default dataset patent details result at $0.0002/result; Apify pricing schedule created with two-week lead time |
...
| `actors/google-trends-autocomplete-scraper` | `google-trends-autocomplete-scraper` | `x4NPccdc4xbJZ2r6c` | Google Trends Autocomplete Scraper | Local source present; live build `yNvoJILR0ohgl4R3h` / `1.0.4` succeeded; q-alias smoke run `7u581w1so9EHbnjcd` succeeded with 5 suggestion rows for `tesla` in `US`; default dataset `6aqp4ICDOwSfsENPw`; `OUTPUT.saved_suggestion_count = 5`; `OUTPUT.raw_response_omitted = false`; `SCRAPPA_API_KEY` is secret on version `1.0` | Public with `PAY_PER_EVENT` pricing scheduled for 2026-06-19T09:30:30.582Z, `suggestion-result` at $0.0002/result; Apify blocked immediate pricing with two-week lead time |
...
| `actors/google-trends-related-queries-scraper` | `google-trends-related-queries-scraper` | `hoF0Qgm3S0wAfpY8y` | Google Trends Related Queries Scraper | Local source present; QA recovery build `pkeoRQeZ6bZLuYVPB` / `1.0.6` succeeded; exact-prefill smoke run `xkuztmDi18JroBxwO` succeeded in 4.2s with 25 dataset rows; autocomplete smoke `GAEO5De2WIcEBckmb` also succeeded; Actor notice verified as `NONE` | Public with active `PAY_PER_EVENT` pricing, `related-result` at $0.0002/result; retryable Scrappa failures honor a deadline-bounded retry budget within the 240s run timeout |
...
| `actors/kleinanzeigen-listing-details-scraper` | `kleinanzeigen-listing-details-scraper` | `1hSNdgPwGINp7xeHB` | Kleinanzeigen Listing Details Scraper | Local source present; live build `yuhYcYVKEgrqygJhk` succeeded; public smoke run `wcCLm7I6kdC973Azh` saved two requested listings (`3348371956`, `3401748259`) | Public with active `PAY_PER_EVENT` pricing from 2026-07-10T11:19:59.585Z (`listing-detail-result` at $0.00025/result); smoke run charged two events |
...
| `actors/redfin-property-details-scraper` | `redfin-property-details-scraper` | `9DsqUcpBF5b66AFOY` | Redfin Property Details Scraper | Local source present; live Apify version uses `SOURCE_FILES`; latest run `tmNPG9p7hzXxCg3Nl` succeeded; latest build `nbQFFsvWZwrjkqXB7` succeeded | Public with `PAY_PER_EVENT` pricing scheduled for 2026-06-09T19:30:02.533Z, `property-result` at $0.0003/result; Apify pricing schedule created with two-week lead time |
...
| `actors/redfin-valuation-scraper` | `redfin-valuation-scraper` | `g4let6UdMxr2Io4uc` | Redfin Valuation Scraper | Local source present; live Apify version uses `SOURCE_FILES`; latest run `HZEf3NX7L6aVO09BW` succeeded; latest build `f12rrIJw7qcHXhFiU` succeeded | Public with `pricingInfos` active from 2026-05-26T19:20:00Z (`PAY_PER_EVENT`, `valuation-result` at $0.0005/result) |
...
| `actors/trustedshops-shop-profile-scraper` | `trustedshops-shop-profile-scraper` | `hhSb9mCZkXYTQc3Lo` | TrustedShops Shop Profile Scraper | Local source present; live Apify version uses `SOURCE_FILES`; latest build `WIXCLT4x1CH7ZjoXz` / `1.0.2` succeeded; smoke run `0hOz53dBI1NfAXIVK` succeeded with 1 dataset item for TSID `XFB15FFBDE1DEE7A55D292A7D48598A6A` (`NeuTrends`, `https://www.neutrends.de`, language `de`, target market `DEU`) | Public with `pricingInfos` active from 2026-06-17T09:48:02Z (`PAY_PER_EVENT`, `shop-profile-result` at $0.0002/result); smoke run charged `shop-profile-result = 1` |
...
| `actors/vinted-user-profile-scraper` | `vinted-user-profile-scraper` | `0z7FbFWBw77KVoabS` | Vinted User Profile Scraper | Local source present; public Actor build `EjLoh2HagHQAvZcjv` / `1.0.2` succeeded; source parity verified across 22 files | Active `PAY_PER_EVENT` pricing verified 2026-07-13 (`user-profile-result` at $0.0005/profile); two-profile and mixed-success smoke runs verified |
...
| `actors/youtube-api-search-data` | `youtube-api-search-data` | `ziD2fUoLsdzKlc6zR` | YouTube Search Data Scraper | Local source present; live Apify version uses `SOURCE_FILES`; QA 503 recovered 2026-09-26 with transient-retry build `0.0.15` (see `reports/2026-09-26-youtube-search-data-qa-recovery.md`) | Scheduled 2026-05-17 (`PAY_PER_EVENT`); verify activation |
