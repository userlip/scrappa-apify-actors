...
- Runtime deployment: build `1.0.5` (`LJwRpNUEYupnGjSJj`) succeeded and passed the production smoke. Documentation-only build `1.0.6` (`PvKU0vx4I9LLXTDhE`) then succeeded and received the `latest` tag after the Store README was updated to lead with batch input; its packaged runtime source is unchanged from `1.0.5`.
