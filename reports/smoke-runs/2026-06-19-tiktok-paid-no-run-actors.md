...
Scope: smoke-run the four public paid TikTok actors flagged by the 2026-06-19 health audit as needing a successful current production run: `tiktok-ads-scraper`, `tiktok-comments-scraper`, `tiktok-hashtag-posts-scraper`, and `tiktok-search-scraper`. Inputs were minimal documented examples and all scraping stayed on Scrappa API infrastructure. No actor source, batching behavior, pricing, deployment, or listing metadata was changed.

Pre-run validation: live Apify API metadata showed all four target actors were public, had `SCRAPPA_API_KEY` present as a secret on version `1.0`, had `PAY_PER_EVENT` pricing entries for the `apify-default-dataset-item` event, and had successful latest builds. The immediate live run-history query returned no runs for the four target actor IDs, but repository history already documented prior successful runs for `tiktok-search-scraper` on 2026-06-10 and `tiktok-hashtag-posts-scraper` on 2026-05-24, so this report should be read as current smoke evidence rather than all-time first-run evidence for every row.

Post-run validation: `APIFY_TOKEN=... node scripts/audit-apify-health.mjs --json` reported the owner-filtered portfolio summary at `32` OK and `47` no-runs. The same audit reported all four target actors as `OK`, each with latest run status `SUCCEEDED`. The task-provided baseline was `51` no-runs; because two target actors had older documented successes in repo history, the safest interpretation is that this sweep refreshed current run evidence for all four target actors and confirmed they are no longer reported as `NO_RUNS` by the live audit.
