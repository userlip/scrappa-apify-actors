...
const FAILED_BUILD_STATUSES = new Set(['FAILED', 'TIMED-OUT', 'ABORTED']);
...
async function runCli() {
  ...
  if (
    report.summary.failedLatestRuns > 0 ||
    report.summary.nonTerminalLatestRuns > 0 ||
    report.summary.failedLatestBuilds > 0 ||
    report.summary.auditErrors > 0
  ) {
    process.exit(1);
  }
}
...
export function createHealthAuditReport({ actors, excludedPublicActors = [], errors = [], checkedAt = new Date() }) {
  ...
  const failedLatestBuildActors = actorReports.filter((report) => report.status === 'FAILED_LATEST_BUILD');
  ...
  return {
    ...
    summary: {
      ...
      failedLatestBuilds: failedLatestBuildActors.length,
...
export function auditActorHealth(input) {
  ...
  if (FAILED_BUILD_STATUSES.has(latestBuild?.status)) {
    return {
      ...base,
      status: 'FAILED_LATEST_BUILD',
      reason: `Latest build status is ${latestBuild.status}.`,
    };
  }
...
function printReport(report) {
  ...
  console.log(`Failed latest builds: ${report.summary.failedLatestBuilds}`);
  ...
  printActorSection('FAILED LATEST BUILDS', report.failedLatestBuildActors, (actor) => {
    return `${actorLabel(actor)} latest build ${actor.latestBuild?.id || 'unknown'} ${actor.latestBuild?.status || 'UNKNOWN'}`;
  });
